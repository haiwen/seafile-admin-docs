...
| `SEAF_SERVER_STORAGE_TYPE`   | What kind of the Seafile data for storage (must be same as in the Seafile server deployment configuration). Available options are `disk` (i.e., local disk), `s3`, `multiple` or left as empty (Read storage settings from seafile.conf) |

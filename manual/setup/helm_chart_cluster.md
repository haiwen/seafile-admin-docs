...
4. Then install the chart use the following command:
    ...
    helm upgrade --install seafile seafile/13.0/cluster  --namespace seafile --create-namespace --values my-values.yaml
    ```
...
3. Upgrade release to a new version
    ...
    ```sh
    helm upgrade --install seafile seafile/13.0/cluster --namespace seafile --create-namespace --values my-values.yaml --version <release-version>
    ```

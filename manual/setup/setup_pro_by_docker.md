...
| ------------------------------- | ------------------------------------------------------------------------------------------------------------- | ------------------------------- |  
| `BASIC_STORAGE_PATH`            | Base path for Docker persistent data | `/opt` |
...
| `INNER_NOTIFICATION_SERVER_URL` | Notification server URL reachable from the Seafile container | `http://notification-server:8083` |
| `ENABLE_METADATA_MANAGEMENT` | Enable metadata management | `false` |
| `INNER_METADATA_SERVER_URL` | Metadata server URL reachable from the Seafile container | (required when metadata management is enabled) |
...
| `SS_MAX_OBJ_CACHE_SIZE` | Maximum SeaSearch object-cache size | `10GB` |
| `SS_LOG_TO_STDOUT` | Write SeaSearch logs to standard output | `false` |
| `SS_LOG_OUTPUT` | Enable SeaSearch log output | `true` |
| `SS_LOG_LEVEL` | SeaSearch log level | `info` |
| `ELASTICSEARCH_SCHEME` | Elasticsearch connection scheme | `http` |
| `ELASTICSEARCH_HOST` | Elasticsearch host | (required when using Elasticsearch) |
| `ELASTICSEARCH_PORT` | Elasticsearch port | `9200` |
| `ELASTICSEARCH_USER` | Elasticsearch username | (none) |
| `ELASTICSEARCH_PASSWORD` | Elasticsearch password | (none) |
| `NON_ROOT`   | Run Seafile container without a root user      | `false` |

The template also defines component image variables, derived storage paths, and SeaSearch settings. For their full descriptions, see [environment variables](../config/env.md), [Search with SeaSearch](./use_seasearch.md), and [Metadata server](../extension/metadata-server.md).

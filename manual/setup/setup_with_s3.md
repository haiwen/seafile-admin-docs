...
### Using IAM Role to Access S3 Storage

Since pro 13.0.6, seafile server supports accessing S3 storage using an **IAM Role** when running on an **AWS EC2 instance**. In this mode, Seafile obtains temporary credentials from the EC2 instance metadata service, so it is **not necessary to configure access\_key and secret\_key**.

In this case, Seafile will automatically retrieve temporary credentials from the EC2 metadata service to access S3.

Enable IAM Role authentication by setting the following environment variables:

| Variable               | Description                                                                             | Default Value |
| ---------------------- | --------------------------------------------------------------------------------------- | ------------- |
| `S3_USE_IAM_ROLE`      | Use IAM Role authentication if enabled.                                                 | false         |
...
* The IAM Role has permission to access the target S3 bucket

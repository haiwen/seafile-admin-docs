...
For Helm Chart 13.0.4 and later, configure one of the following options in `my-values.yaml` and run `helm upgrade`. Select one method unless you intentionally need two routes.
...
1. Record the generated logs to the standard output. In this way, the logs can be distinguished under each node by `kubectl logs` (but all types of logs will be output together now). For K8S resource-file deployments, enable this feature by modifying `SEAFILE_LOG_TO_STDOUT` to `true` in `seafile-env.yaml`:
    ...
    Current Helm Chart 13.0 values enable `SEAFILE_LOG_TO_STDOUT: "true"` by default for single-node and cluster deployments. Helm users should change `seafile.env.SEAFILE_LOG_TO_STDOUT` in `my-values.yaml` and run `helm upgrade` rather than edit `seafile-env.yaml`.
